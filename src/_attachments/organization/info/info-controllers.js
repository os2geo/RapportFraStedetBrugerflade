        ...
        function ($scope, $rootScope, $http, $stateParams, $upload) {
            ...
            $scope.update = function (form) {
                ...
                if (form.$valid && !$scope.imageerror) {
                    $scope.organization.type = "organization";
                    $http.put('/couchdb/' + $rootScope.appID + '/' + $stateParams.organization, $scope.organization).
                    success(function (data, status, headers, config) {
                        $scope.organization._rev = data.rev;
                        $scope.success = data;
                        if (file) {
                            var fileReader = new FileReader();
                            ...
                            fileReader.onload = function (e) {
                                $upload.http({
                                    method: 'PUT',
                                    url: '/couchdb/' + $rootScope.appID + '/' + $stateParams.organization + '/logo?rev=' + data.rev,
                                    headers: {
