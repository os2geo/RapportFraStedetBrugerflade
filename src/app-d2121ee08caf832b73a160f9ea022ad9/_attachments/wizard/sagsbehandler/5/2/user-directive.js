(function (window, angular, console) {
    'use strict';
    /* Directives */
    angular.module('myApp.directives').directive("emailuser", function ($compile, $http, $stateParams) {
        return {
            restrict: "E",
            scope: {
                user: '=',
            ...
            templateUrl: 'wizard/sagsbehandler/5/2/user.html',
            controller: function ($scope, $rootScope) {
                ...
                var buildKeys = function (node, parent) {
                    for (var key in node.properties) {
                        if(key==='properties'){
                            for(var k in node.properties.properties){
                                $scope.keys.push(k);
                                $scope.ruletype[k] = node.properties.properties[k];
                            }
                        }
                ...
                $http.get('/couchdb/db-' + $rootScope.wizard.template.sagsbehandler.overlay.database + '/_design/schema').success(function (data, status, headers, config) {
