(function (window, angular, console) {
    'use strict';
    angular.module('myApp.controllers').controller('wizard-sagsbehandler-6', ['$scope', '$rootScope', '$http', '$stateParams', '$state', '$modal',
        function ($scope, $rootScope, $http, $stateParams, $state, $modal) {
            ...
            $scope.removeOverlay = function (overlay) {
                for (var i = 0; i < $rootScope.wizard.baselayers.sagsbehandler.length; i++) {
                    if ($rootScope.wizard.baselayers.sagsbehandler[i].$$hashKey === overlay.$$hashKey) {
                        $rootScope.wizard.baselayers.sagsbehandler.splice(i, 1);
                        break;
                    }
                }
            }
            ...
            $scope.startChanged = function (item) {
                console.log(item);
                for (var i = 0; i < $rootScope.wizard.baselayers.sagsbehandler.length; i++) {
                    if ($rootScope.wizard.baselayers.sagsbehandler[i].$$hashKey !== item.$$hashKey) {
                        $rootScope.wizard.baselayers.sagsbehandler[i].overlay.selected = false;
                    }
                }
            };
            ...
            $scope.next = function () {
                ...
                var selected = false;
                for (var i = 0; i < $rootScope.wizard.baselayers.sagsbehandler.length; i++) {
                    if ($rootScope.wizard.baselayers.sagsbehandler[i].overlay.selected === true) {
                        selected = true;
                        break;
                    }
                }
                if ($rootScope.wizard.baselayers.sagsbehandler.length === 0) {
                    ...
                    $state.go('wizard.sagsbehandler.7');
                }
            }
        }
    ]);
