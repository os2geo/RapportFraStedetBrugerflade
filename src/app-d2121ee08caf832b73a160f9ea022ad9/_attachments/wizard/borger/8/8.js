(function (window, angular, console) {
    'use strict';
    angular.module('myApp.controllers').controller('wizard-borger-8', ['$scope', '$rootScope', '$http', '$stateParams', '$state', '$modal',
        function ($scope, $rootScope, $http, $stateParams, $state, $modal) {
            ...
            $scope.removeOverlay = function (overlay) {
                for (var i = 0; i < $rootScope.wizard.overlays.borger.length; i++) {
                    if ($rootScope.wizard.overlays.borger[i].$$hashKey === overlay.$$hashKey) {
                        $rootScope.wizard.overlays.borger.splice(i, 1);
                        break;
                    }
                }
            }
